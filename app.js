(() => {
  ...
  const RUNNER_STATE_KEY = `student_form_runner_state_${(window.APP_CONFIG?.FORM_SLUG || 'student-registration')}_v3`;
  ...
  const DEFAULT_FORM = {
    ...
    questions: [
      ...
      {id:'student_child_order',label:'دانش‌آموز فرزند چندم است',type:'number',required:false,options:[],placeholder:'مثلاً ۲ یا اول',validation:{min:1,max:30,step:1,integer:true},conditions:[],conditionMode:'and'},
  ...
  function isBlankAnswer(value) {
    ...
    return normalizeText(String(value)).length === 0;
  }

  function normalizePatternInput(q, value) {
    ...
    if (/(شماره|تلفن|موبایل|phone|mobile|tel)/i.test(label)) {
      return text.replace(/[\s\-–—()]+/g, '');
    }
    return text;
  }

  const PERSIAN_ORDINALS = Object.freeze({
    'اول':1,'اولین':1,'یکم':1,'یک':1,
    'دوم':2,'سوم':3,'چهارم':4,'پنجم':5,'ششم':6,'هفتم':7,'هشتم':8,'نهم':9,'دهم':10,
    'یازدهم':11,'دوازدهم':12,'سیزدهم':13,'چهاردهم':14,'پانزدهم':15,'پانزدهم':15,
    'شانزدهم':16,'هفدهم':17,'هجدهم':18,'نوزدهم':19,'بیستم':20,
    'بیست و یکم':21,'بیست و دوم':22,'بیست و سوم':23,'بیست و چهارم':24,'بیست و پنجم':25,
    'بیست و ششم':26,'بیست و هفتم':27,'بیست و هشتم':28,'بیست و نهم':29,'سی‌ام':30,'سی ام':30,'سی‌امین':30
  });
  ...
  function parseOrdinalNumber(value) {
    ...
    const numeric = parseFlexibleNumber(raw);
    if (Number.isFinite(numeric)) return numeric;

    const cleaned = raw
      .replace(/^(فرزند|دانش آموز|دانش‌آموز|نفر)\s*/,'')
      .trim();

    if (Object.prototype.hasOwnProperty.call(PERSIAN_ORDINALS, cleaned)) {
      return PERSIAN_ORDINALS[cleaned];
    }
  ...
  function persistRunnerState() {
    try {
      const payload = {
        ...
        current: Number(state.runner.current) || 0,
        values: clone(state.runner.values || {}),
        pendingVersion: state.runner.pendingVersion ? clone(state.runner.pendingVersion) : null,
        deviceName: state.runner.deviceName || ''
      };
      sessionStorage.setItem(RUNNER_STATE_KEY, JSON.stringify(payload));
    } catch (error) {
      console.warn('Could not persist runner state:', error);
    }
  }

  function restoreRunnerState() {
    try {
      const raw = sessionStorage.getItem(RUNNER_STATE_KEY);
      if (!raw) return;
      const saved = JSON.parse(raw);
      if (!saved || typeof saved !== 'object') return;
      state.runner.version = saved.version ? saved.version : null;
      state.runner.current = Number.isInteger(saved.current) ? Math.max(0, saved.current) : 0;
      state.runner.values = saved.values && typeof saved.values === 'object' ? saved.values : {};
      state.runner.pendingVersion = saved.pendingVersion || null;
      if (typeof saved.deviceName === 'string') state.runner.deviceName = saved.deviceName;
    } catch (error) {
      console.warn('Could not restore runner state:', error);
      sessionStorage.removeItem(RUNNER_STATE_KEY);
    }
  }

  function clearRunnerState() {
    try { sessionStorage.removeItem(RUNNER_STATE_KEY); } catch {}
  }
  ...
  const ui={
    ...
    error:(message)=>openModal({title:'خطا',message:Array.isArray(message)?message.filter(Boolean).join('\n'):String(message ?? ''),buttons:[{label:'باشه',value:true,kind:'danger'}],closeOnBackdrop:true}),
  ...
  const errorController = {
    last: null,
    normalize(error, fallback='عملیات ناموفق بود.') {
      if (error instanceof Error) {
        return {message: error.message || fallback, details: error.stack || '', code: error.code || '', raw: error};
      }
      if (Array.isArray(error)) {
        return {message: error.filter(Boolean).join('\n') || fallback, details: '', code: '', raw: error};
      }
      if (typeof error === 'string') {
        return {message: error.trim() || fallback, details: '', code: '', raw: error};
      }
      if (error && typeof error === 'object') {
        return {
          message: error.message || error.error_description || error.detail || fallback,
          details: error.details || error.hint || error.context || '',
          code: error.code || '',
          raw: error
        };
      }
      return {message: fallback, details: '', code: '', raw: error};
    },
    show(error, {title='خطا', fallback='عملیات انجام نشد.', includeDetails=false, context=''} = {}) {
      const normalized = this.normalize(error, fallback);
      this.last = { ...normalized, title, context, at: nowIso() };
      console.error(`[${context || title}]`, normalized.raw || error);
      const parts = [normalized.message];
      if (includeDetails && normalized.code) parts.push(`کد خطا: ${normalized.code}`);
      if (includeDetails && normalized.details) parts.push(normalized.details);
      return ui.error(parts.join('\n'));
    },
    showValidation(errors, title='اعتبارسنجی') {
      const items = Array.isArray(errors) ? errors.filter(Boolean) : [errors].filter(Boolean);
      if (!items.length) return null;
      const body = [`${title} با خطا مواجه شد:`, ...items.map((item, index) => `${faNumber(index + 1)}. ${item}`)].join('\n');
      return ui.error(body);
    }
  };
  ...
  function normalizeSubmissionValues(form, values) {
    ...
    if (clean.family_children_count !== undefined && clean.student_child_order !== undefined) {
      clean.family_children_count = clean.family_children_count;
      clean.student_child_order = clean.student_child_order;
    }
    return clean;
  }
  ...
  function collectResponseErrors(form, values) {
    ...
    const countRaw = values.family_children_count;
    const orderRaw = values.student_child_order;
    if (!isBlankAnswer(countRaw) && !isBlankAnswer(orderRaw)) {
      const count = parseFlexibleNumber(countRaw);
      let order = parseFlexibleNumber(orderRaw);
      if (!Number.isFinite(order)) order = parseOrdinalNumber(orderRaw);
  ...
  function validateQuestion(q, values, questions = null) {
    if (!q) return null;
    if (questions && !isQuestionVisible(q, values)) return null;

    const value = values?.[q.id];
    const empty = isBlankAnswer(value);
    if (q.required && empty) {
      if (q.type === 'checkbox') return `حداقل یک گزینه برای «${q.label}» انتخاب کنید.`;
      return `پاسخ «${q.label}» الزامی است.`;
    }
    ...
    if (q.type === 'number') {
      ...
      if (!Number.isFinite(n) && q.id === 'student_child_order') {
        const ordinal = parseOrdinalNumber(value);
        if (Number.isFinite(ordinal)) n = ordinal;
      }
      if (!Number.isFinite(n)) return `مقدار «${q.label}» باید عدد معتبر باشد.`;
    ...
    if (['text', 'textarea', 'select', 'radio'].includes(q.type)) {
      ...
      if (v.pattern) {
        try {
          ...
          const candidate = normalizePatternInput(q, text);
          const compact = candidate.replace(/[\s\-–—()]+/g, '');
          if (!regex.test(candidate) && !regex.test(compact)) return `قالب «${q.label}» صحیح نیست.`;
        } catch {
          return `قالب اعتبارسنجی سؤال «${q.label}» خراب است.`;
        }
      }
    }

    if (q.type === 'select' || q.type === 'radio') {
      if (q.options.length && !q.options.includes(String(value))) return `گزینه انتخاب‌شده برای «${q.label}» معتبر نیست.`;
    }

    if (q.type === 'checkbox') {
      if (!Array.isArray(value) || value.some(vv => !q.options.includes(String(vv)))) return `گزینه‌های انتخاب‌شده برای «${q.label}» معتبر نیستند.`;
    }

    if (q.type === 'date') {
      const text = normalizeDigits(String(value ?? '')).trim();
      if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) return `تاریخ «${q.label}» معتبر نیست.`;
    }

    if (q.type === 'jalali') {
      if (!isValidJalaliDate(value)) return `تاریخ شمسی «${q.label}» معتبر نیست.`;
    }

    if (q.type === 'serial') {
      const raw = normalizeDigits(String(value ?? '')).replace(/\s+/g, '');
      if (raw && !new RegExp(`^[${SERIAL_LETTERS.join('')}][0-9]{2}\/[0-9]{6}$`).test(raw)) return `فرمت سریال «${q.label}» صحیح نیست.`;
    }

    return null;
  }
  function validateAllResponse(form,values) {
    const errors=collectResponseErrors(form,values);
  ...
  function helpText(q){
    ...
    if (q.type === 'date') return 'تاریخ میلادی را انتخاب کنید. اعداد فارسی نیز به‌درستی پذیرفته می‌شوند.';
    if (q.type === 'serial') return 'قالب: حرف + دو رقم / شش رقم؛ مثال: ا12/123456';
    if (q.type === 'number') return q.id === 'student_child_order'
      ? 'عدد فارسی/انگلیسی یا ترتیب نوشتاری مانند «اول»، «دوم»، «سوم» را وارد کنید.'
      : 'عدد فارسی یا انگلیسی معتبر وارد کنید.';
  ...
  function renderQuestionInput(q,value,prefix='public') {
    ...
    if (q.type === 'number') {
      const isChildOrder = q.id === 'student_child_order';
      const placeholder = isChildOrder ? (q.placeholder || 'مثلاً ۲ یا اول') : (ph || 'مثلاً ۱۲۳');
      const inputMode = isChildOrder ? 'numeric' : ((q.validation || {}).integer ? 'numeric' : 'decimal');
      return `<input id="${id}" class="input numeric-input" type="text" inputmode="${inputMode}" autocomplete="off" value="${escapeHtml(value ?? '')}" placeholder="${escapeHtml(placeholder)}" />`;
    }
    ...
    if (q.type === 'serial') {
      const m = String(value || '').match(/^(.)(\d{2})\/(\d{6})$/);
      const a = m ? m[1] : '', b = m ? m[2] : '', c = m ? m[3] : '';
    ...
    if (q.type === 'checkbox') {
      ...
      return `<div class="option-list">${q.options.map(o=>`<label class="option-item"><input type="checkbox" data-multi="${escapeHtml(id)}" value="${escapeHtml(o)}" ${arr.includes(o) ? 'checked' : ''} /><span>${escapeHtml(o)}</span></label>`).join('')}</div>`;
    }
  ...
  function readAnswer(q,container,prefix) {
    ...
    if (q.type === 'serial') {
      const letter = $(id + '_1', container)?.value || '';
      const d2 = normalizeDigits($(id + '_2', container)?.value || '').replace(/[^0-9]/g, '').slice(0, 2);
      const d6 = normalizeDigits($(id + '_3', container)?.value || '').replace(/[^0-9]/g, '').slice(0, 6);
      if (!letter && !d2 && !d6) return '';
      return `${letter}${d2}/${d6}`;
    }
    if (q.type === 'number') {
      const raw = normalizeDigits($(id, container)?.value ?? '').trim();
      if (q.id === 'student_child_order') {
        const ordinal = parseOrdinalNumber(raw);
        if (Number.isFinite(ordinal)) return String(ordinal);
      }
      return raw.replace(/[\s,،\u00a0]/g, '');
    }
    if (q.type === 'text' && /(?:شماره|تلفن|موبایل|phone|mobile|tel)/i.test(normalizeText(q.label || ''))) {
      return normalizeDigits($(id, container)?.value ?? '').replace(/[\s\-–—()]+/g, '').trim();
    }
  ...
  function bindAnswer(q,container,prefix,onChanged,{autoNext=false}={}) {
    const id=makeAnswerId(prefix,q.id);
    const save=()=>{onChanged(q,readAnswer(q,container,prefix));};
  ...
  function formatReviewValue(q, value) {
    if (Array.isArray(value)) {
      return value.length ? value.join('، ') : 'بدون پاسخ';
    }
  ...
  function openReviewPreview(form, values) {
    const root = $('reviewRoot');
    const list = $('reviewQuestionList');
    if (!root || !list) return Promise.resolve(false);

    const visible = visibleQuestions(form, values);
    const visibleIds = new Set(visible.map(q => q.id));
    const allQuestions = Array.isArray(form.questions) ? form.questions : visible;
    list.innerHTML = '';
    ...
    allQuestions.forEach((q, index) => {
      ...
      label.textContent = `${faNumber(index + 1)}. ${q.label}`;

      const value = document.createElement('div');
      value.className = 'review-value';

      if (!visibleIds.has(q.id)) {
        value.textContent = 'این سؤال به دلیل شرایط نمایش برای این پاسخ نمایش داده نشد.';
        value.classList.add('review-empty');
      } else {
        const answer = formatReviewValue(q, values[q.id]);
        value.textContent = answer;
        if (answer === 'بدون پاسخ') value.classList.add('review-empty');
      }
  ...
  async function finishPublic() {
    ...
    if(errors.length)return errorController.showValidation(errors,'اعتبارسنجی فرم عمومی');
    ...
    }catch(error){errorController.show(error,{title:'ثبت اطلاعات',fallback:'ثبت اطلاعات ناموفق بود.',includeDetails:true,context:'finishPublic'});}
  ...
  async function loadPublicForm({silent=false}={}) {
    ...
    if (!silent) {
      $('publicLoading').classList.remove('hidden');
      $('publicUnavailable').classList.add('hidden');
    ...
    try {
      ...
      if (!data) {
        ...
        $('publicUnavailable').classList.remove('hidden');
        $('publicUnavailableMessage').textContent = 'هنوز هیچ نسخه‌ای از فرم توسط مدیر منتشر نشده است.';
        setConnection('منتشر نشده', 'neutral');
        return;
      }
      ...
      const hasCurrentVersion = !!state.runner.version;
      const versionChanged = !hasCurrentVersion || state.runner.version.id !== version.id;
      const hasUserProgress = Object.keys(state.runner.values || {}).length > 0 || state.runner.current > 0;

      if (versionChanged) {
        if (hasCurrentVersion && hasUserProgress) {
          state.runner.pendingVersion = version;
          $('publicUpdateBanner').classList.remove('hidden');
        } else {
          state.runner.version = version;
          state.runner.pendingVersion = null;
      ...
      $('publicLoading').classList.add('hidden');
      $('publicUnavailable').classList.remove('hidden');
      $('publicUnavailableMessage').textContent = error.message || 'خطا در دریافت فرم.';
      setConnection('خطای اتصال', 'danger');
      if (!silent) ui.error(error.message || 'دریافت فرم ناموفق بود.');
    }
  }

  function subscribePublicRealtime(formId) {
    // عمدی: فرم عمومی در طول تکمیل پاسخ، به‌صورت خودکار دوباره بارگیری نمی‌شود.
    // بررسی نسخه جدید فقط با دکمه «بررسی نسخه جدید» انجام می‌شود تا فرم کاربر هرگز ریست نشود.
  }

  function loadPendingVersion(){
    if(!state.runner.pendingVersion)return;
    state.runner.version=state.runner.pendingVersion;
    state.runner.pendingVersion=null;
    state.runner.values={};
    state.runner.current=0;
    $('publicUpdateBanner').classList.add('hidden');
  ...
  function serialValueValid(value){return !value || new RegExp(`^[${SERIAL_LETTERS.join('')}][0-9]{2}\\/[0-9]{6}$`).test(normalizeDigits(String(value)).replace(/\s+/g,''));}
  ...
  async function saveEditedResponse() {
    ...
    const err=validateAllResponse(form,cleanedValues);if(err)return ui.error(err);
    try{
    ...
    }catch(error){errorController.show(error,{title:'به‌روزرسانی پاسخ',fallback:'به‌روزرسانی پاسخ ناموفق بود.',includeDetails:true,context:'saveEditedResponse'});}
  }
  ...
  async function bootPublic(){
    restoreRunnerState();
    $('runnerDeviceName').value=state.runner.deviceName;
    if (state.runner.version) {
      renderPublicRunner();
      $('publicLoading').classList.add('hidden');
      $('publicUnavailable').classList.add('hidden');
      $('runnerPanel').classList.remove('hidden');
    }
    await loadPublicForm();
  }
