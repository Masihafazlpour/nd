(() => {
  ...
  function validateQuestion(q,values,questions=q?null:null) {
    ...
    if(['text','textarea','select','radio'].includes(q.type)){
      ...
      if(v.pattern){try{if(!new RegExp(v.pattern).test(normalizeDigits(text)))return `قالب «${q.label}» صحیح نیست.`;}catch{return `قالب اعتبارسنجی سؤال «${q.label}» خراب است.`;}}
    }
  ...
  async function finishPublic() {
    ...
    const error=validateAllResponse(form,state.runner.values);if(error)return ui.error(error);
    ...
    try{
      const {data,error:rpcError}=await state.sb.rpc('submit_public_response',{p_form_id:form.form_id,p_form_version_id:form.id,p_response:state.runner.values,p_device_name:state.runner.device_name||''});
      if(rpcError)throw rpcError;
  ...
  async function saveEditedResponse() {
    ...
    const form={questions:state.editSchema}; const err=validateAllResponse(form,state.editResponse.values);if(err)return ui.error(err);
    try{
      state.editResponse.values=pruneHiddenValues({questions:state.editSchema},state.editResponse.values);
